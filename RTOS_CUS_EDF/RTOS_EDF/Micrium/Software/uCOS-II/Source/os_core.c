...
// Update all task's priority via EDF
void UpdateEDFPriorities(void) {
    OS_TCB* ptcb;
    EDFEntry  EDF_deadline_order[MAX];
	int current_task = OSTCBCur->OSTCBId;
    int count = 0;
    // Create EDF list
    ptcb = OSTCBList;
    while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
    ...
    for (int j = 0; j < numTasks; j++) {
        printf("Priority is change from %d to %d\n", taskList[EDF_deadline_order[j].id - 1]->OSTCBPrio, EDF_deadline_order[j].newpriority);
        // Change the priority of each task according to the sorted priority
