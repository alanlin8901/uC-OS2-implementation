...
void OS_MakeReady(void) {
    OS_TCB* ptcb;
    OS_ENTER_CRITICAL();
    ptcb = OSTCBList;
    // Make all input task to READY
    while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
        if (ptcb->OSTCBArriveTime == OSTimeGet()) {
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        OSTimeSet(0);                                /* Set the clock time when OSStart mulri-task     */
        // Print the header for the task information
...
INT16U CurrentTaskID;
INT32U CurrentTaskNumber;
INT32U CompletionResponseTime;
INT32U CompletionPreemptionTime;
INT32U CompletionTimeDly;
INT32U RemainTime;
INT8U CheckEvent;
INT16U CurrentAperiodicTaskID;
INT32U AperiodicAbsolabeDeadline;
INT32U AperiodicCalculatedDeadline;
INT32U DynamicCUSDeadline = 0;
INT32U SuspendPriority = 0;
INT32U CompleteApriodicFinishedTime = 0;
INT32U CompleteApriodicCalculatedDeadline = 0;
INT8U ArrivedAperiodicJobCounter = 0;
INT8U StealHaveAperiodicJob = 0;
INT8U AperiodicNumber = 0;
INT8U ArriveAndSetCounter = 0;
INT8U ArriveAndSetTaskID;
INT8U NumberMissAbsoluteDeadlineTask = 0;
INT8U NumberCompleteAperiodicTask = 0;
INT8U AperiodicFlag = 0;
INT8U DoNothing = 0;
BOOLEAN MissAbsoluteDeadline = 0;
BOOLEAN ArriveAndSetDeadline = 0;
BOOLEAN OnlySetDeadline = 0;
INT8U DoNothingIDStack[OS_MAX_TASKS];
INT8U MissDeadlineIDStack[OS_MAX_TASKS];
...
// Clear all task's priority
void ClearAllTaskPriority(void) {
    ...
    for (int i = 0; i < numTasks - 1; i++) {
        for (int j = i + 1; j < numTasks; j++) {
            if (taskList[i]->OSTCBPrio < taskList[j]->OSTCBPrio) {
                // Swap task locations
                OS_TCB* temp = taskList[i];
    ...
    // Adjust the priority of tasks according to the order of priority
    int i = 1;
    for (int j = 0; j < numTasks; j++) {
        // Change the priority of each task according to the sorted priority
        OSTaskChangePrio(taskList[j]->OSTCBPrio, OS_TASK_IDLE_PRIO - i++);
    }
}

// Comparison function: sort by deadline, if the same, sort by id (from small to large)
int compare_tasks_via_deadline(const void* a, const void* b) {
    EDFEntry* task1 = (EDFEntry*)a;
    EDFEntry* task2 = (EDFEntry*)b;

    if (task1->deadline != task2->deadline) {
        return task1->deadline - task2->deadline;  // Sort by deadline
    }
    else {
        return task1->id - task2->id;  // If deadlines are the same, sort by id in descending order
    }
}

// Update all task's priority via CUS
void UpdateEDFPriorities(void) {
    OS_TCB* ptcb;
    EDFEntry  EDF_deadline_order[MAX];
    int count = 0;
    int current_susepnd_task_priority = -1;
    // Create CUS list
    ptcb = OSTCBList;
    while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
        if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == 0) { // Only add READY/RUNNING tasks
            EDFEntry entry;
            entry.id = ptcb->OSTCBId;
            if (ptcb->OSTCBId != TASK_NUMBER)
                entry.deadline = ptcb->OSTCBArriveTime + ptcb->OSTCBPeriodic * ptcb->OSTCBNumber + ptcb->OSTCBPeriodic;
            else
                entry.deadline = ptcb->OSTCBCalculatedDeadline;
            EDF_deadline_order[count++] = entry;
        }
        else {
			current_susepnd_task_priority = ptcb->OSTCBPrio;
        }
        ptcb = ptcb->OSTCBNext;
    }
    // First sort by deadline from small to large. If the deadlines are the same, sort by id from small to large.
    qsort(EDF_deadline_order, count, sizeof(EDFEntry), compare_tasks_via_deadline);
    // Update the priority of each task
    for (int i = 0; i < count; i++) {
        if (current_susepnd_task_priority <= i)
            EDF_deadline_order[i].newpriority = i + 1;
        else
            EDF_deadline_order[i].newpriority = i;
    }
    // Reset all task's priority
    ClearAllTaskPriority();
    // Assign the priority of each task
    OS_TCB* taskList[MAX];
    ptcb = OSTCBList;
    int numTasks = 0;
    int i = 0;
    ...
    for (int j = 0; j < numTasks; j++)
        // Change the priority of each task according to the sorted priority
        OSTaskChangePrio(taskList[EDF_deadline_order[j].id - 1]->OSTCBPrio, EDF_deadline_order[j].newpriority);
}

void taskEnterCheckMissabsAndArrSetDeadline(void) {

    OS_TCB* ptcb;
    while (AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].TaskArriveTime == 0 && 
        AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].CalculatedDeadline  >  0) {
        // Check if the aperiodic task is missed absolute deadline
        if (AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].CalculatedDeadline >
            AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].AbsoluteDeadline) {
            NumberMissAbsoluteDeadlineTask++;
            MissDeadlineIDStack[MissAbsoluteDeadline++] = AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].ApeirodicTaskID;
        }
        else {
            if (!AperiodicFlag) {
                ArriveAndSetDeadline = 1;
                DynamicCUSDeadline = AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].CalculatedDeadline;
                ArriveAndSetTaskID = AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].ApeirodicTaskID;
                ArriveAndSetCounter = ArrivedAperiodicJobCounter;
                // Set new aperiodic task deadline
                ptcb = OSTCBList;
                while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
                    if (ptcb->OSTCBId == TASK_NUMBER) {
                        ptcb->OSTCBArriveTime = AperiodicTaskParameter[ArrivedAperiodicJobCounter].TaskArriveTime;
                        ptcb->OSTCBExecutionTime = AperiodicTaskParameter[ArrivedAperiodicJobCounter].TaskExecutionTime;
                        ptcb->OSTCBAbsoluteDeadline = AperiodicTaskParameter[ArrivedAperiodicJobCounter].AbsoluteDeadline;
                        ptcb->OSTCBCalculatedDeadline = AperiodicTaskParameter[ArrivedAperiodicJobCounter].CalculatedDeadline;
                        ptcb->OSTCBPeriodic = AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].TaskPeriodic;
                        ptcb->OSTCBApeirodicTaskID = AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].ApeirodicTaskID;
                        ptcb->OSTCBCPUTime = 0;
                        OSTaskResume(SuspendPriority);
                        UpdateEDFPriorities();
                    }
                    ptcb = ptcb->OSTCBNext;
                }
                ...
                DoNothingIDStack[DoNothing++] = AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].ApeirodicTaskID;
                StealHaveAperiodicJob++;
            }
        }
        ArrivedAperiodicJobCounter++;
    }
}
void  OSTimeTick(void)
{
...
#endif
    int next_task_id = OSTCBCur->OSTCBId;
    int next_task_number = OSTCBCur->OSTCBNumber;
    int next_apeioridc_task_number = OSTCBCur->OSTCBApeirodicTaskID;
	// print inital aperiodic task information
    if (OSTimeGet() == 0) {
        taskEnterCheckMissabsAndArrSetDeadline();
    }
    if (OSTimeGet() > 0 && OSTimeGet() < 41) {
        if ((CurrentTaskID != next_task_id || CurrentTaskNumber != next_task_number) && CompletionTimeDly >= 0 
            && !(next_task_id == TASK_NUMBER && CurrentTaskID == TASK_NUMBER)) {
            if (next_task_id == 65535) {
                if (AperiodicCalculatedDeadline > 0) {
                    printf("%2d\tAperiodic job(%d) is finished.\n",
                        OSTimeGet(), AperiodicNumber);
                    printf("%2d\tCompletion\t  task(%2d)(%2d)\t  task(63)\t\t %d\t\t %d\t\tN/A\n",
                        OSTimeGet(), CurrentTaskID, AperiodicNumber, CompletionResponseTime, CompletionPreemptionTime);
                    if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
                        fprintf(Output_fp, "%2d\tAperiodic job(%d) is finished.\n",
                            OSTimeGet(), AperiodicNumber);
                        fprintf(Output_fp, "%2d\tCompletion\t  task(%2d)(%2d)\t  task(63)\t\t %d\t\t %d\t\tN/A\n",
                            OSTimeGet(), CurrentTaskID, AperiodicNumber, CompletionResponseTime, CompletionPreemptionTime);
                        fclose(Output_fp);
                    }
                    AperiodicFlag--;
                    ...
                    printf("%2d\tCompletion\t  task(%2d)(%2d)\t  task(63)\t\t %d\t\t %d\t\t%d\n",
                        OSTimeGet(), CurrentTaskID, CurrentTaskNumber, CompletionResponseTime, CompletionPreemptionTime, CompletionTimeDly);
                    if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
                        fprintf(Output_fp, "%2d\tCompletion\t  task(%2d)(%2d)\t  task(63)\t\t %d\t\t %d\t\t%d\n",
                            OSTimeGet(), CurrentTaskID, CurrentTaskNumber, CompletionResponseTime, CompletionPreemptionTime, CompletionTimeDly);
                        fclose(Output_fp);
                    }
                }
            }
            else if (CurrentTaskID == 65535) {
                if (next_task_id != TASK_NUMBER) {
                    printf("%2d\tPreemption\t  task(63)\t  task(%2d)(%2d)\n",
                        OSTimeGet(), next_task_id, next_task_number);
                    if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
                        fprintf(Output_fp, "%2d\tPreemption\t  task(63)\t  task(%2d)(%2d)\n",
                            OSTimeGet(), next_task_id, next_task_number);
                        fclose(Output_fp);
                    }
                }
                    ...
                    printf("%2d\tPreemption\t  task(63)\t  task(%2d)(%2d)\n",
                        OSTimeGet(), next_task_id, next_apeioridc_task_number);
                    if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
                        fprintf(Output_fp, "%2d\tPreemption\t  task(63)\t  task(%2d)(%2d)\n",
                            OSTimeGet(), next_task_id, next_apeioridc_task_number);
                        fclose(Output_fp);
                ...
                if (CheckEvent == COMPLETION) {
                    if (AperiodicCalculatedDeadline > 0) {
                        printf("%2d\tAperiodic job(%d) is finished.\n",
                            OSTimeGet(), AperiodicNumber);
                        printf("%2d\tCompletion\t  task(%2d)(%2d)\t  task(%2d)(%2d)\t\t %d\t\t %d\t\tN/A\n",
                            OSTimeGet(), CurrentTaskID, AperiodicNumber, next_task_id, next_task_number, CompletionResponseTime, CompletionPreemptionTime);
                        if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
                            fprintf(Output_fp, "%2d\tAperiodic job(%d) is finished.\n",
                                OSTimeGet(), AperiodicNumber);
                        ...
                        if (next_task_id == TASK_NUMBER) {
                            printf("%2d\tCompletion\t  task(%2d)(%2d)\t  task(%2d)(%2d)\t\t %d\t\t %d\t\t%d\n",
                                OSTimeGet(), CurrentTaskID, CurrentTaskNumber, next_task_id, next_apeioridc_task_number, CompletionResponseTime, CompletionPreemptionTime, CompletionTimeDly);
                            if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
                            ...
                            printf("%2d\tCompletion\t  task(%2d)(%2d)\t  task(%2d)(%2d)\t\t %d\t\t %d\t\t%d\n",
                                OSTimeGet(), CurrentTaskID, CurrentTaskNumber, next_task_id, next_task_number, CompletionResponseTime, CompletionPreemptionTime, CompletionTimeDly);
                            if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
                                fprintf(Output_fp, "%2d\tCompletion\t  task(%2d)(%2d)\t  task(%2d)(%2d)\t\t %d\t\t %d\t\t%d\n",
                                    OSTimeGet(), CurrentTaskID, CurrentTaskNumber, next_task_id, next_task_number, CompletionResponseTime, CompletionPreemptionTime, CompletionTimeDly);
                                fclose(Output_fp);
                    ...
                    if (CurrentTaskID != TASK_NUMBER) {
                        printf("%2d\tPreemption\t  task(%2d)(%2d)\t  task(%2d)(%2d)\n",
                            OSTimeGet(), CurrentTaskID, CurrentTaskNumber, next_task_id, next_task_number);
                        if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
                        ...
                        printf("%2d\tPreemption\t  task(%2d)(%2d)\t  task(%2d)(%2d)\n",
                            OSTimeGet(), CurrentTaskID, CurrentAperiodicTaskID, next_task_id, next_task_number);
                        if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
                            fprintf(Output_fp, "%2d\tPreemption\t  task(%2d)(%2d)\t  task(%2d)(%2d)\n",
                                OSTimeGet(), CurrentTaskID, CurrentAperiodicTaskID, next_task_id, next_task_number);
                            fclose(Output_fp);
    ...
    while (MissAbsoluteDeadline) {
        printf("%2d\tAperiodic job(%d) arrives. But can't scheduled. (miss absolute deadline)\n",
            OSTimeGet(), MissDeadlineIDStack[NumberMissAbsoluteDeadlineTask - MissAbsoluteDeadline]);
        if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
            fprintf(Output_fp, "%2d\tAperiodic job(%d) arrives. But can't scheduled. (miss absolute deadline)\n",
                OSTimeGet(), MissDeadlineIDStack[NumberMissAbsoluteDeadlineTask - MissAbsoluteDeadline]);
            fclose(Output_fp);
        }
        MissAbsoluteDeadline--;
    }
    if (ArriveAndSetDeadline) {
        printf("%2d\tAperiodic job(%d) arrives and sets CUS server's deadline as %2d.\n",
            OSTimeGet(), ArriveAndSetTaskID, AperiodicTaskCrossFunctionParameter[ArriveAndSetCounter].CalculatedDeadline);
        if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
            fprintf(Output_fp, "%2d\tAperiodic job(%d) arrives and sets CUS server's deadline as %2d.\n",
                OSTimeGet(), ArriveAndSetTaskID, AperiodicTaskCrossFunctionParameter[ArriveAndSetCounter].CalculatedDeadline);
            fclose(Output_fp);
        }
        ArriveAndSetDeadline = !ArriveAndSetDeadline;
    }
    if (OnlySetDeadline) {
        printf("%2d\tAperiodic job(%d) sets CUS server's deadline as %d.\n",
            OSTimeGet(), NumberCompleteAperiodicTask,
            AperiodicTaskCrossFunctionParameter[NumberCompleteAperiodicTask].CalculatedDeadline);
        if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
        ...
        OnlySetDeadline = !OnlySetDeadline;
    }
    while (DoNothing) {
    ...
    OSTime++;
    OSTCBCur->OSTCBCPUTime++;
    OS_TRACE_TICK_INCREMENT(OSTime);
    OS_EXIT_CRITICAL();
#endif
    if (OSRunning == OS_TRUE) {
#if OS_TICK_STEP_EN > 0u
		// setting the system end time for the OS
        ...
        CurrentTaskID = OSTCBCur->OSTCBId;
        CurrentAperiodicTaskID = OSTCBCur->OSTCBApeirodicTaskID;
        CurrentTaskNumber = OSTCBCur->OSTCBNumber;
        CompletionResponseTime = OSTimeGet() - OSTCBCur->OSTCBNumber * OSTCBCur->OSTCBPeriodic - OSTCBCur->OSTCBArriveTime;
        CompletionPreemptionTime = CompletionResponseTime - OSTCBCur->OSTCBExecutionTime;
        CompletionTimeDly = OSTCBCur->OSTCBPeriodic + OSTCBCur->OSTCBNumber * OSTCBCur->OSTCBPeriodic - OSTimeGet() + OSTCBCur->OSTCBArriveTime;
        RemainTime = OSTCBCur->OSTCBExecutionTime - OSTCBCur->OSTCBCPUTime;
        if (RemainTime == 0 && DynamicCUSDeadline > 0 && OSTCBCur->OSTCBCalculatedDeadline > 0) { // If is budget task need compute another
            CurrentTaskNumber = AperiodicNumber;
            CompletionResponseTime = OSTimeGet() - OSTCBCur->OSTCBArriveTime;
            if (OSTCBCur->OSTCBArriveTime <= CompleteApriodicFinishedTime) {
                CompletionPreemptionTime = OSTimeGet() - CompleteApriodicCalculatedDeadline - OSTCBCur->OSTCBExecutionTime;
            }
            else {
                CompletionPreemptionTime = OSTimeGet() - OSTCBCur->OSTCBArriveTime - OSTCBCur->OSTCBExecutionTime;
            }
            CompletionTimeDly = OSTCBCur->OSTCBPeriodic - OSTimeGet() + OSTCBCur->OSTCBArriveTime;
        }
        AperiodicAbsolabeDeadline = OSTCBCur->OSTCBAbsoluteDeadline;
        AperiodicCalculatedDeadline = OSTCBCur->OSTCBCalculatedDeadline;
        OS_EXIT_CRITICAL();

        OS_ENTER_CRITICAL();
        //Check deadline in each delay time of task
        ptcb = OSTCBList;
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
			if (ptcb->OSTCBId != TASK_NUMBER) { // Check missdeadline if it is not CUS server
                int CheckDlyTime = ptcb->OSTCBPeriodic + ptcb->OSTCBArriveTime + ptcb->OSTCBNumber * ptcb->OSTCBPeriodic - OSTimeGet();
                if ((int)(CheckDlyTime) < 0 && CurrentTaskID != 65535) {
                    printf("%2d\tMissDeadline\t  task(%2d)(%2d)\t  -------------------\n",
                        OSTimeGet() - 1, ptcb->OSTCBId, ptcb->OSTCBNumber);
                    if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
        ...
        OS_ENTER_CRITICAL();
        // Set input task to READY if tocuh arrived time
        ptcb = OSTCBList;
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
            if (ptcb->OSTCBArriveTime == OSTimeGet()) {
                OSRdyGrp |= ptcb->OSTCBBitY;
        ...
        // Set new aperiodic task deadline if there is no aperiodic task by checking arrive time
        while (AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].TaskArriveTime == OSTimeGet()) {
            // Check if the aperiodic task is missed absolute deadline
            if (AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].CalculatedDeadline >
                AperiodicTaskCrossFunctionParameter[ArrivedAperiodicJobCounter].AbsoluteDeadline) {
                NumberMissAbsoluteDeadlineTask++;
        ...
        // Set new aperiodic task deadline if it arrived before CUS server deadline
        if (DynamicCUSDeadline == OSTimeGet()) {
            if (StealHaveAperiodicJob) {
                OSTaskResume(SuspendPriority);
                AperiodicFlag++;
                OnlySetDeadline = 1;
                DynamicCUSDeadline = AperiodicTaskCrossFunctionParameter[NumberCompleteAperiodicTask].CalculatedDeadline;
                OS_ENTER_CRITICAL();
                // Set new aperiodic task deadline
                ptcb = OSTCBList;
                while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
                    if (ptcb->OSTCBId == TASK_NUMBER) {
                        ptcb->OSTCBArriveTime = AperiodicTaskParameter[APERIODIC_TASK_NUMBER - StealHaveAperiodicJob].TaskArriveTime;
                        ptcb->OSTCBExecutionTime = AperiodicTaskParameter[APERIODIC_TASK_NUMBER - StealHaveAperiodicJob].TaskExecutionTime;
                        ptcb->OSTCBAbsoluteDeadline = AperiodicTaskParameter[APERIODIC_TASK_NUMBER - StealHaveAperiodicJob].AbsoluteDeadline;
                        ptcb->OSTCBCalculatedDeadline = AperiodicTaskParameter[APERIODIC_TASK_NUMBER - StealHaveAperiodicJob].CalculatedDeadline;
                        ptcb->OSTCBPeriodic = AperiodicTaskCrossFunctionParameter[APERIODIC_TASK_NUMBER - StealHaveAperiodicJob].TaskPeriodic;
                        ptcb->OSTCBApeirodicTaskID = AperiodicTaskCrossFunctionParameter[APERIODIC_TASK_NUMBER - StealHaveAperiodicJob].ApeirodicTaskID;
                        ptcb->OSTCBCPUTime = 0;
                        UpdateEDFPriorities();
                    }
                    ptcb = ptcb->OSTCBNext;
        ...
        // No other aperiodic task set budget for idle
        if (DynamicCUSDeadline == OSTimeGet()) {
            // Set the aperiodic task to suspend and give bigger number of them to avoid some issue
            ptcb = OSTCBList;
            while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
                if (ptcb->OSTCBId == TASK_NUMBER) { // Check missdeadline if it is CUS server
                    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == 0) {
                        int CheckDlyTime;
                        if ((int)(ArrivedAperiodicJobCounter - NumberCompleteAperiodicTask - StealHaveAperiodicJob - NumberMissAbsoluteDeadlineTask) > 0) {
                            // Check missdaedline time
                            CheckDlyTime = ptcb->OSTCBCPUTime - ptcb->OSTCBExecutionTime;
                        }
                        else {
                            CheckDlyTime = 0;
                        }
                        if ((int)(CheckDlyTime) < 0 && CurrentTaskID != 65535) {
                            printf("%2d\tMissDeadline\t  task(%2d)(%2d)\t  -------------------\n",
                                OSTimeGet(), ptcb->OSTCBId, AperiodicNumber);
                            if (Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a") == 0) {
                                fprintf(Output_fp, "%2d\tMissDeadline\t  task(%2d)(%2d)\t  -------------------\n",
                                    OSTimeGet(), ptcb->OSTCBId, AperiodicNumber);
                                fclose(Output_fp);
                    ...
                    SuspendPriority = ptcb->OSTCBPrio;
                    OSTaskSuspend(SuspendPriority);
                    UpdateEDFPriorities();
                }
                ptcb = ptcb->OSTCBNext;
            }
        ...
        if (RemainTime != 0)
            CheckEvent = PREEMPTION;
        else {
            CheckEvent = COMPLETION;
            OSTCBCur->OSTCBNumber++;
            if (OSTCBCur->OSTCBId == TASK_NUMBER) {
                AperiodicNumber = OSTCBCur->OSTCBApeirodicTaskID;
                NumberCompleteAperiodicTask++;
                CompleteApriodicFinishedTime = OSTimeGet();
                CompleteApriodicCalculatedDeadline = OSTCBCur->OSTCBCalculatedDeadline;
            }
        }
                   ...
                   INT16U   opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
		...
		ptcb->OSTCBNumber        = 0u;					   /* Clear number of task being created       */
        ...
        ptcb->OSTCBCPUTime       = 0u;                     /* Task Execution time is 0                 */
        task_para_set* task_data = (task_para_set*)pext;
        if (prio != OS_TASK_IDLE_PRIO) {
            ptcb->OSTCBArriveTime = task_data->TaskArriveTime;        /* Store arrive time                        */
            ptcb->OSTCBExecutionTime = task_data->TaskExecutionTime;  /* Store execution time                     */
            ptcb->OSTCBPeriodic = task_data->TaskPeriodic;		      /* Store periodic time                      */
            ptcb->OSTCBAbsoluteDeadline = task_data->AbsoluteDeadline;/* Store AbsoluteDeadline                   */
            ptcb->OSTCBCalculatedDeadline = task_data->CalculatedDeadline;/* Store CalculatedDeadline             */
			ptcb->OSTCBApeirodicTaskID = task_data->ApeirodicTaskID;/* Store AperiodicTaskID                   */
        }
#else
        ...
        OS_TRACE_TASK_READY(ptcb);
        OS_EXIT_CRITICAL();
        /*printf("------After TCB[%2.1d] begin linked------\n", (ptcb->OSTCBPrio == 63) ? 63 : (ptcb->OSTCBPrio + 1));
        printf("Previous TCB point to adress\t%6.1x\n", ptcb->OSTCBPrev);
        printf("Current  TCB point to adress\t%6.1x\n", (void*)ptcb);
        printf("Next     TCB point to adress\t%6.1x\n\n", ptcb->OSTCBNext);*/
        return (OS_ERR_NONE);
    }
    OS_EXIT_CRITICAL();
