...
*********************************************************************************************************
*/
// If we need to change next task, we enter this function, consider two case, 1. next higher priority task enter 2.the current task is finished and there is a lower priority task is ready queue
...
void  OSTimeDly (INT32U ticks)
{
	...
	if (OSTCBCur->OSTCBCPUTime == OSTCBCur->OSTCBExecutionTime) {
        OSTCBCur->OSTCBCPUTime = 0;
        // Calculate the reamin time for the current task need to delay
        int remain_time = OSTCBCur->OSTCBNumber * ticks - OSTimeGet() + OSTCBCur->OSTCBArriveTime;
        if (OSTCBCur->OSTCBCalculatedDeadline > 0) {
            remain_time = OSTCBCur->OSTCBCalculatedDeadline - OSTimeGet();
        }
		//  Change tick to remain time for the current task
        if (remain_time > 0u) {                            /* 0 means no delay!                         */
            OS_ENTER_CRITICAL();
            ...
            OSTCBCur->OSTCBDly = remain_time;              /* Load ticks in TCB                          */
            OS_TRACE_TASK_DLY(remain_time);
            OS_EXIT_CRITICAL();
