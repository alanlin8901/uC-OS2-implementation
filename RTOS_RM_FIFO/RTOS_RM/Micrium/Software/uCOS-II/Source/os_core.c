...
void  OSTimeTick(void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        CompletionTimeDly = OSTCBCur->OSTCBPeriodic + OSTCBCur->OSTCBArriveTime + OSTCBCur->OSTCBNumber * OSTCBCur->OSTCBPeriodic - OSTimeGet();
        ...
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
            if (ptcb->OSTCBArriveTime == OSTimeGet()) {
                ...
                OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
                ptcb->OSTCBStat = OS_STAT_RDY;
                OS_TRACE_TASK_READY(highest_ptcb);
            }
            ptcb = ptcb->OSTCBNext;
        }
