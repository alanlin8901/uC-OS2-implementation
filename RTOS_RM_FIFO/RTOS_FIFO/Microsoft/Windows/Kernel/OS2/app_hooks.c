...
void PriorityAssignment() {
	// Re-assign the initial priority for each task by FIFO_PRIORITY
	INT32U for_default_priority = 0;
	for (int k = 0; k < TASK_NUMBER; k++) {
		if (TaskParameter[k].TaskArriveTime == 0) {
			TaskParameter[k].TaskPriority = FIFO_PRIORITY++;
			for_default_priority = FIFO_PRIORITY;
		}
		else {
			TaskParameter[k].TaskPriority = 63 - for_default_priority++;
		}
	}
}
