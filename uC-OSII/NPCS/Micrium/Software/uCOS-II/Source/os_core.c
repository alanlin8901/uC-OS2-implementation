...
void  OSTimeTick(void)
{
    ...
    if (!recordR1 && LockR1 && UseR1) {
        printf("%2d\tLockResource\ttask(%2d)(%2d)\t R1\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber);
        record_local("%2d\tLockResource\ttask(%2d)(%2d)\t R1\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber);
        recordR1 = 1;
    }
    if (!recordR2 && LockR2 && UseR2) {
        printf("%2d\tLockResource\ttask(%2d)(%2d)\t R2\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber);
        record_local("%2d\tLockResource\ttask(%2d)(%2d)\t R2\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber);
        recordR2 = 1;
    }
    if (!recordR1 && !LockR1 && UseR1) {
        printf("%2d\tUnlockResource\ttask(%2d)(%2d)\t R1\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber);
        record_local("%2d\tUnlockResource\ttask(%2d)(%2d)\t R1\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber);
        recordR1 = 1;
    }
    if (!recordR2 && !LockR2 && UseR2) {
        printf("%2d\tUnlockResource\ttask(%2d)(%2d)\t R2\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber);
        record_local("%2d\tUnlockResource\ttask(%2d)(%2d)\t R2\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber);
        recordR2 = 1;
    }
    if (OSTimeGet() > 0 && OSTimeGet() < 101) {
        if ((CurrentTaskID != next_task_id || CurrentTaskNumber != next_task_number) && CompletionTimeDly >= 0 && !LockR1 && !LockR2) {
    ...
    if (OSRunning == OS_TRUE) {
        ...
        OSTCBCur->OSTCBCPUTime;
		// Check if the task is in the critical section
        ...
        if (NeedComputeBlockTime) {
            ...
            while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
                if (ptcb->OSTCBPrio < OSTCBCur->OSTCBPrio) {
                    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {
                        if (ptcb->OSTCBPeriodic * ptcb->OSTCBNumber + ptcb->OSTCBArriveTime < OSTimeGet()) {
                            if (LowestBlockingPriority > ptcb->OSTCBPrio) {
                                LowestBlockingPriority = ptcb->OSTCBPrio;
