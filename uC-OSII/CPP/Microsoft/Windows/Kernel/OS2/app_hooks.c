...
void ComputeResourceCeilings(task_para_set* TaskParameter, int TASK_NUMBER, int* R1_ceiling, int* R2_ceiling) {
	int minPriority_R1 = 9999; // Assume large
	int minPriority_R2 = 9999;

	for (int i = 0; i < TASK_NUMBER; i++) {
		if (TaskParameter[i].R1LockTime != 0 || TaskParameter[i].R1UnLockTime != 0) {
			if (TaskParameter[i].TaskPriority < minPriority_R1) {
				minPriority_R1 = TaskParameter[i].TaskPriority;
			}
		}

		if (TaskParameter[i].R2LockTime != 0 || TaskParameter[i].R2UnLockTime != 0) {
			if (TaskParameter[i].TaskPriority < minPriority_R2) {
				minPriority_R2 = TaskParameter[i].TaskPriority;
			}
		}
	}

	if (minPriority_R1 != 9999) {
		*R1_ceiling = minPriority_R1 - R1_PRIO; // R1 index = 1
	}
	else {
		*R1_ceiling = -1; // Not used
	}

	if (minPriority_R2 != 9999) {
		*R2_ceiling = minPriority_R2 - R2_PRIO; // R2 index = 2
	}
	else {
		*R2_ceiling = -1;
	}
...
void InputFile(void)
{
	...
	// Assign priority to R1 and R2 Ceiling
	ComputeResourceCeilings(TaskParameter, TASK_NUMBER, &R1_ceiling, &R2_ceiling);
