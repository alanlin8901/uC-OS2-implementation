...
void OS_MakeReady(void) {
    ...
    while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
        if (ptcb->OSTCBArriveTime == OSTimeGet()) {
			...
			ptcb->OSTCBDly = ptcb->OSTCBArriveTime;
        }
        ptcb = ptcb->OSTCBNext;
    }
    OS_EXIT_CRITICAL();
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        ...
        printf("Tick\tEvent\t\tCurrentTask ID\t  NextTask ID\t  ResponseTime\t  BlockingTime\t  PerrmptionTime\n");
        OS_MakeReady();
...
INT32U CurrentTaskPriority;
INT32U CompletionResponseTime;
INT32U CompletionPreemptionTime;
INT32U CompletionTimeDly;
INT32U RemainTime;
INT8U CheckEvent;
INT32U R1_want_lock_time;
INT32U R2_want_lock_time;
INT32U R1_want_unlock_time;
INT32U R2_want_unlock_time;
INT32U UseR1;
INT32U UseR2;
INT32U recordR1;
INT32U recordR2;
INT32U R1LockLocker;
INT32U R1UnLockLocker;
INT32U R2LockLocker;
INT32U R2UnLockLocker;
INT32U ResourceBlockingTime;
INT32U ResourceBlockingPriority;
INT32U NeedComputeBlockTime;
INT32U BothLock;
void record_local(const char* format, ...) {
    FILE* fp;
    int err;
    va_list args;
    err = fopen_s(&fp, OUTPUT_FILE_NAME, "a");
    if (err == 0) {
...
void  OSTimeTick(void)
{
    ...
    if (CurrentTask != NULL) {
        if (CurrentTask->OSTCBLockFirst == R1_PRIO) {
            if (!LockR2 && !BothLock)
                curr_task_priority = CurrentTask->OSTCBEnterR1;
            else if (!LockR2 && BothLock)
                curr_task_priority = R1_ceiling;
            else if (LockR2 && !BothLock)
                curr_task_priority = CurrentTask->OSTCBExitR1;
            else if (LockR2 && BothLock)
                curr_task_priority = R1_ceiling;
        }
        if (CurrentTask->OSTCBLockFirst == R2_PRIO) {
            if (!LockR1 && !BothLock)
                curr_task_priority = CurrentTask->OSTCBEnterR2;
            else if (!LockR1 && BothLock)
                curr_task_priority = R2_ceiling;
            else if (LockR1 && !BothLock)
                curr_task_priority = CurrentTask->OSTCBExitR2;
            else if (LockR1 && BothLock)
                curr_task_priority = R2_ceiling;
        }
        if (!LockR1 && !LockR2)
            curr_task_priority = CurrentTask->OSTCBPrio;
    }
    if (!recordR1 && LockR1 && UseR1) {
        printf("%2d\tLockResource\ttask(%2d)(%2d)\t R1\t%d to %d\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber, curr_task_priority, R1_ceiling);
        record_local("%2d\tLockResource\ttask(%2d)(%2d)\t R1\t%d to %d\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber, curr_task_priority, R1_ceiling);
        recordR1 = 1;
    }
    if (!recordR2 && LockR2 && UseR2) {
        printf("%2d\tLockResource\ttask(%2d)(%2d)\t R2\t%d to %d\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber, curr_task_priority, R2_ceiling);
        record_local("%2d\tLockResource\ttask(%2d)(%2d)\t R2\t%d to %d\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber, curr_task_priority, R2_ceiling);
        recordR2 = 1;
    }
    if (!recordR1 && !LockR1 && UseR1) {
        printf("%2d\tUnlockResource\ttask(%2d)(%2d)\t R1\t%d to %d\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber, R1_ceiling, curr_task_priority);
        record_local("%2d\tUnlockResource\ttask(%2d)(%2d)\t R1\t%d to %d\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber, R1_ceiling, curr_task_priority);
        if (!LockR2) CurrentTask->OSTCBLockFirst = 0;
        recordR1 = 1;
    }
    if (!recordR2 && !LockR2 && UseR2) {
        printf("%2d\tUnlockResource\ttask(%2d)(%2d)\t R2\t%d to %d\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber, R2_ceiling, curr_task_priority);
        record_local("%2d\tUnlockResource\ttask(%2d)(%2d)\t R2\t%d to %d\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber, R2_ceiling, curr_task_priority);
        if (!LockR1) CurrentTask->OSTCBLockFirst = 0;
        recordR2 = 1;
    }
    if (OSTimeGet() > 0 && OSTimeGet() < 101) {
        if ((CurrentTaskID != next_task_id || CurrentTaskNumber != next_task_number) && CompletionTimeDly >= 0) {
            if (next_task_id == 65535) {
                int PreemptionWithoutBlocking = CompletionPreemptionTime;
                int BlockingTime = 0;
                if (CurrentTask->OSTCBPrio < ResourceBlockingPriority) {
                    PreemptionWithoutBlocking -= CurrentTask->OSTCBBlockingTime;
                    BlockingTime = CurrentTask->OSTCBBlockingTime;
                    CurrentTask->OSTCBBlockingTime = 0;
                }
                printf("%2d\tCompletion\ttask(%2d)(%2d)\t  task(63)\t\t %d\t\t %d\t\t%d\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber, CompletionResponseTime, BlockingTime, PreemptionWithoutBlocking);
                record_local("%2d\tCompletion\ttask(%2d)(%2d)\t  task(63)\t\t %d\t\t %d\t\t%d\n", OSTimeGet(), CurrentTaskID, CurrentTaskNumber, CompletionResponseTime, BlockingTime, PreemptionWithoutBlocking);
                CurrentTask->OSTCBLockFirst = 0;
            }
            else if (CurrentTaskID == 65535) {
                printf("%2d\tPreemption\ttask(63)\t  task(%2d)(%2d)\n", OSTimeGet(), next_task_id, next_task_number);
                record_local("%2d\tPreemption\ttask(63)\t  task(%2d)(%2d)\n", OSTimeGet(), next_task_id, next_task_number);
            }
            else {
                if (CheckEvent == COMPLETION) {
                    ...
                    printf("%2d\tCompletion\ttask(%2d)(%2d)\t  task(%2d)(%2d)\t\t %d\t\t %d\t\t%d\n",
                        OSTimeGet(), CurrentTaskID, CurrentTaskNumber, next_task_id, next_task_number, CompletionResponseTime, BlockingTime, PreemptionWithoutBlocking);
                    record_local("%2d\tCompletion\ttask(%2d)(%2d)\t  task(%2d)(%2d)\t\t %d\t\t %d\t\t%d\n",
                        OSTimeGet(), CurrentTaskID, CurrentTaskNumber, next_task_id, next_task_number, CompletionResponseTime, BlockingTime, PreemptionWithoutBlocking);
                    CurrentTask->OSTCBLockFirst = 0;
                }
                else {
                    ...
                    record_local("%2d\tPreemption\ttask(%2d)(%2d)\t  task(%2d)(%2d)\n",
                        OSTimeGet(), CurrentTaskID, CurrentTaskNumber, next_task_id, next_task_number);
                }
            }
        }
    ...
    if (OSRunning == OS_TRUE) {
        ...
        OSTCBCur->OSTCBCPUTime;
		// Check if task is in the critical section
        if (OSTCBCur->OSTCBCPUTime == OSTCBCur->OSTCBR1Lock && !LockR1 && !R1LockLocker) {
            R1_want_lock_time = OSTimeGet();
            R1LockLocker = 1;
        }
        if (OSTCBCur->OSTCBCPUTime == OSTCBCur->OSTCBR2Lock && !LockR2 && !R2LockLocker) {
            R2_want_lock_time = OSTimeGet();
            R2LockLocker = 1;
        }
        if (OSTCBCur->OSTCBCPUTime == OSTCBCur->OSTCBR1UnLock && LockR1 && !R1UnLockLocker) {
            R1_want_unlock_time = OSTimeGet();
            R1UnLockLocker = 1;
        }
        if (OSTCBCur->OSTCBCPUTime == OSTCBCur->OSTCBR2UnLock && LockR2 && !R2UnLockLocker) {
            R2_want_unlock_time = OSTimeGet();
            R2UnLockLocker = 1;
        }
        ...
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
            int CheckDlyTime;
            CheckDlyTime = ptcb->OSTCBPeriodic + ptcb->OSTCBArriveTime + ptcb->OSTCBNumber * ptcb->OSTCBPeriodic - OSTimeGet();
            if ((int)(CheckDlyTime) < 0 && CurrentTaskID != 65535) {
                printf("%2d\tMissDeadline\t  task(%2d)(%2d)\t  ------------\n",
                    OSTimeGet() - 1, ptcb->OSTCBId, ptcb->OSTCBNumber);
                record_local("%2d\tMissDeadline\t  task(%2d)(%2d)\t  ------------\n",
                    OSTimeGet() - 1, ptcb->OSTCBId, ptcb->OSTCBNumber);
                OSRunning = OS_FALSE;
                exit(0);
            }
        ...
        // Make sure no other highest priority task need to do before lock mutex
        int OtherDoFirst = 0;
        ptcb = OSTCBList;
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
            if (ptcb->OSTCBPrio < OSTCBCur->OSTCBPrio){
                if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {
                    if (ptcb->OSTCBPeriodic * ptcb->OSTCBNumber + ptcb->OSTCBArriveTime == OSTimeGet()) {
                        OtherDoFirst = 1;
                    }
                }
            }
		...
		// If task is not in the critical section
        if (!OtherDoFirst) {
            // If task need lock resource
            if (OSTimeGet() == R1_want_lock_time) {
                LockR1 = 1;
                recordR1 = 0;
                UseR1 = 1;
                R1LockLocker = 0;
                if (LockR2)
                    BothLock = 1;
                if (!OSTCBCur->OSTCBLockFirst)
                    OSTCBCur->OSTCBLockFirst = 1;
                if (OSTCBCur->OSTCBPrio > ResourceBlockingPriority)
                    ResourceBlockingPriority = OSTCBCur->OSTCBPrio;
                OSTCBCur->OSTCBEnterR1 = OSTCBCur->OSTCBPrio;
                OSTaskChangePrio(OSTCBCur->OSTCBPrio, R1_ceiling);
            }
            if (OSTimeGet() == R2_want_lock_time) {
                LockR2 = 1;
                recordR2 = 0;
                UseR2 = 1;
                R2LockLocker = 0;
                if (LockR1)
                    BothLock = 1;
                if (!OSTCBCur->OSTCBLockFirst)
                    OSTCBCur->OSTCBLockFirst = 2;
                if (OSTCBCur->OSTCBPrio > ResourceBlockingPriority)
                    ResourceBlockingPriority = OSTCBCur->OSTCBPrio;
                OSTCBCur->OSTCBEnterR2 = OSTCBCur->OSTCBPrio;
                OSTaskChangePrio(OSTCBCur->OSTCBPrio, R2_ceiling);
            }
        }

        // If task need unlock resource
        if (OSTimeGet() == R1_want_unlock_time) {
            LockR1 = 0;
            recordR1 = 0;
            NPCSR1ReleaseTime = OSTimeGet();
            R1UnLockLocker = 0;
            if (!LockR2)
                BothLock = 0;
            OSTCBCur->OSTCBExitR1 = OSTCBCur->OSTCBPrio;
            OSTaskChangePrio(OSTCBCur->OSTCBPrio, OSTCBCur->OSTCBEnterR1);
        }
        if (OSTimeGet() == R2_want_unlock_time) {
            LockR2 = 0;
            recordR2 = 0;
            NPCSR2ReleaseTime = OSTimeGet();
            R2UnLockLocker = 0;
            if (!LockR1)
                BothLock = 0;
            OSTCBCur->OSTCBExitR2 = OSTCBCur->OSTCBPrio;
            OSTaskChangePrio(OSTCBCur->OSTCBPrio, OSTCBCur->OSTCBEnterR2);
        }

		// Set the blocking time for the task
        ptcb = OSTCBList;
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
            if (ptcb->OSTCBPrio + 2 < OSTCBCur->OSTCBOrigPrio) {
                if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {
                    if (ptcb->OSTCBPeriodic * ptcb->OSTCBNumber + ptcb->OSTCBArriveTime == OSTimeGet()) {
                        if (LockR2 || LockR1)
                            NeedComputeBlockTime = 1;
                    }
                }
            }
            ptcb = ptcb->OSTCBNext;
		...
		// If the task is blocked, set the blocking time
        if (NeedComputeBlockTime) {
            int LowestBlockingPriority = 255;
            ptcb = OSTCBList;
            while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
                if (ptcb->OSTCBPrio + 2 < OSTCBCur->OSTCBOrigPrio) {
                    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {
                        if (ptcb->OSTCBPeriodic * ptcb->OSTCBNumber + ptcb->OSTCBArriveTime < OSTimeGet()) {
                            if (LowestBlockingPriority > ptcb->OSTCBOrigPrio) {
                                LowestBlockingPriority = ptcb->OSTCBOrigPrio;
                                ptcb->OSTCBBlockingTime += 1;
                            }
                        }
                    }
                }
                   ...
                   INT16U   opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        ptcb->OSTCBOrigPrio = prio;                   /* Load task priority into TCB              */
        ...
        ptcb->OSTCBBlockingTime = 0u;
        ptcb->OSTCBLockFirst = 0u;
        ptcb->OSTCBEnterR1 = 0u;
        ptcb->OSTCBEnterR2 = 0u;
        ptcb->OSTCBExitR1 = 0u;
        ptcb->OSTCBExitR2 = 0u;
        ptcb->OSTCBLockFirst = 0u;
        ...
        if (prio != OS_TASK_IDLE_PRIO) {
            ...
            ptcb->OSTCBR1Lock = task_data->R1LockTime;            /* Store R1 lock time                      */
            ptcb->OSTCBR2Lock = task_data->R2LockTime;            /* Store R2 lock time                      */
            ptcb->OSTCBR1UnLock = task_data->R1UnLockTime;          /* Store R1 unlock time                    */
            ptcb->OSTCBR2UnLock = task_data->R2UnLockTime;          /* Store R2 unlock time                    */
        }
#else
